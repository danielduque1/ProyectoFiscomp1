...
lc_hole = 0.0015;   // muy fino cerca del agujero y del cobre
lc_gem  = 0.010;   // fino alrededor de Kapton/cobre
...
// -------------------------------------------------------------------------
// Idea:
//   - Refinamiento moderado en toda la lamina GEM.
//   - Refinamiento fuerte solo cerca de las esquinas donde estan los agujeros.
//   - No usar Field[2] comentado.

// Campo 1: refinamiento moderado alrededor de toda la lamina GEM.
// Esto mantiene una malla razonable en Kapton/cobre, pero no exageradamente fina.
Field[1] = Box;
Field[1].VIn  = lc_gem;
Field[1].VOut = lc_gas;
Field[1].XMin = -eps;
...
Field[1].YMin = -eps;
Field[1].YMax = Ly + eps;
Field[1].ZMin = -kapton/2 - metal - 0.020;
Field[1].ZMax = +kapton/2 + metal + 0.020;

// Campo 2: refinamiento fuerte en el agujero de la esquina inferior izquierda.
Field[2] = Box;
Field[2].VIn  = lc_hole;
Field[2].VOut = lc_gas;
Field[2].XMin = -eps;
Field[2].XMax = r_rim + 0.006;
Field[2].YMin = -eps;
Field[2].YMax = r_rim + 0.006;
Field[2].ZMin = -kapton/2 - metal - 0.060;
Field[2].ZMax = +kapton/2 + metal + 0.060;

// Campo 3: refinamiento fuerte en el agujero de la esquina superior derecha.
Field[3] = Box;
Field[3].VIn  = lc_hole;
Field[3].VOut = lc_gas;
Field[3].XMin = Lx - r_rim - 0.006;
Field[3].XMax = Lx + eps;
Field[3].YMin = Ly - r_rim - 0.006;
Field[3].YMax = Ly + eps;
Field[3].ZMin = -kapton/2 - metal - 0.060;
Field[3].ZMax = +kapton/2 + metal + 0.060;

// Campo final: Gmsh toma el tamaño mas pequeño entre los tres campos.
Field[4] = Min;

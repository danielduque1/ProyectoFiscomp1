...
def main():
    ...
    Am_energies = [106465, 102175, 119930, 123560]
    # weights = [0.585, 0.318, 0.071, 0.026]
    weights = [0.35, 0.3, 0.2, 0.15]  

    # Histograma para la versión colimada
    hEspectro = ROOT.TH1F("hEspectro", "Espectro Am Colimado (r=0.8);Electrones;Frecuencia", 100, 0, 500000)

    # 5. Bucle con escritura dinámica y re-intento
    n_eventos = 1500
    print(f"\n--- Iniciando captura de {n_eventos} eventos COLIMADOS (r = 0.8 cm) ---")

    with open("datos_espectro_am_1500prob.csv", "w", newline="") as f:
        writer = csv.writer(f)
        ...
        for i in tqdm(range(n_eventos), desc="Progreso Colimado"):
            ...
            while total_electrons == 0:
                photon_energy = random.choices(Am_energies, weights=weights, k=1)[0]
    ...
    hEspectro.Draw()
    cHist.SaveAs("resolucion_am_1500prob.png")
